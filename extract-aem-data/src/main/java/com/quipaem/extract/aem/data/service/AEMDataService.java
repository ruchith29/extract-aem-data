...
import com.quipaem.extract.aem.data.utilities.MongoUtilities;
...
public class AEMDataService {
    ...
    public Map<String,List<Map<String,String>>> storeData(String jsonData ,String clientName) throws JsonProcessingException {
        ...
        MongoTemplate userMongoTemplate=mongoUtilities.mongoTemplate(clientName);
        ...
        for (Iterator<Map.Entry<String, JsonNode>> databases = databaseContent.fields(); databases.hasNext(); ) {
            ...
            for (JsonNode innerData : pageData) {
                ...
                result.put("user-id", clientName);
                result.put("domain-name", "");
                Document document=new Document(result);
                if(mongoUtilities.insertData(Constants.COLLECTION_NAME, document,userMongoTemplate)){
                    successCount++;
    ...
    public Map<String,Map<String,List<Document>>> getData(ArrayNode sitePaths,String clientName){
        ...
        for(int i=0;i<sitePaths.size();i++){
            ...
            List<Document> allData=mongoUtilities.getAllData(query,Constants.COLLECTION_NAME,userMongoTemplate);
    ...
    public Map<String,String> updateData(ObjectNode siteData,String clientName) {
        ...
        if (mongoUtilities.updateDocument(document,userMongoTemplate)){
    ...
    public Map<String,Map<String,Map<String,List<Document>>>> getAllData(String clientName) {
        ...
        List<Document> documentList=mongoUtilities.getAllData(query,Constants.COLLECTION_NAME,userMongoTemplate);
    ...
    public ObjectNode clearData(String clientName) {
        MongoTemplate userMongoTemplate=mongoUtilities.mongoTemplate(clientName);

        Map<String,String> response=new LinkedHashMap<>();
        if(clientName.isEmpty()){
            response.put("status","failed");
            response.put("response","user-id is empty");
            return objectMapper.convertValue(response, ObjectNode.class);
        }
        if (mongoUtilities.deleteData(clientName,userMongoTemplate)){
            response.put("status","success");
            response.put("response","data found and cleared");
            return objectMapper.convertValue(response, ObjectNode.class);
        }
        response.put("status","failed");
        response.put("response","No data found with user-id: "+clientName);
        return objectMapper.convertValue(response, ObjectNode.class);
    }
}